...
# 基础环境变量
harbor_addr='kubernetes-register.sswang.com'
http_type='https'
curl_option=$([ "${http_type}" == "https" ] && echo "-k")
api_url='api/v2.0'
...
projects_list=$(curl ${http_type}://${harbor_addr}/${project_api} -s ${curl_option} | jq '.[].name' | awk -F '"' '{print $2}')
for project in ${projects_list}; do
  # 获取所有的镜像名称
  images_list=$(curl ${http_type}://${harbor_addr}/${image_api}${project} -s ${curl_option} | jq '.repository[].repository_name' | awk -F'"|/' '{print $3}')
  for image in ${images_list}; do
    # 获取指定镜像的标签
    tag_list=$(curl ${http_type}://${harbor_addr}/${project_api}/${project}/repositories/${image}/artifacts -s ${curl_option} | jq '.[].tags[].name' | awk -F'"' '{print $2}')
